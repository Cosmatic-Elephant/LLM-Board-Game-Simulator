# 작업 진행 기록 (아카이브 — 세션 1~10)

세션 1~10의 상세 작업 기록이다. 세션 11부터는 `PROGRESS.md`에 간결하게 기록한다.
...
이번 세션은 "멀티플레이 게임 로직 구현"이라는 하나의 큰 틀 아래, 세션 9에서 미뤄뒀던 실제 Socket.io 연동을 처음부터 끝까지 순차적으로 구현하는 중이다. 방 생성부터 라운드 정산까지 서버 권위(server-authoritative) 방식으로 전부 이어졌으며, 아직 세션이 끝나지 않아 이후 대화에서 계속 이어간다.

### 완료한 작업

#### 실제 Socket.io 방 생성/참가/퇴장 (`server.ts`, `src/app/page.tsx`)

- `create-room`/`join-room`: 호스트를 슬롯 0에 등록, 게스트는 입장 순서대로 빈 슬롯에 동적 배정(세션 9의 `GUEST_OWN_SLOT_INDEX` 하드코딩 미결 사항 해소)
- 연결 해제/`leave-room` 공통 정리 로직(`removeParticipant`): 게스트 이탈 시 슬롯만 비우고 `room-update` 브로드캐스트, **호스트 이탈 시 방 전체를 삭제**하고 남은 클라이언트에 `room-closed`를 보내 자동으로 "호스트가 방을 이탈했습니다" 안내 화면으로 전환
- 게임 시작 후(`room.started`) 신규 참가 시도는 `'게임이 꽉 찼습니다'`/`'게임이 진행 중입니다'`로 차단
- 게스트 화면 방 타이틀도 서버가 내려주는 실제 호스트 이름으로 교체 완료(세션 9 미결 사항 해소)

#### 색상 / AI 모델 / 게임 설정 실시간 동기화

- `update-color`/`update-model`/`update-settings` → `colors-update`/`models-update`/`room-update`로 방 전체 브로드캐스트(발신자 포함, 서버가 단일 소스). 호스트가 전체 슬롯 색상+AI 모델 편집 가능, 게스트는 자기 슬롯 색상만 가능 — 나머지는 읽기 전용으로 항상 서버 값을 그대로 반영
- 참가자가 없는 슬롯은 **AI 슬롯**(모델 콤보박스 + AI 뱃지 점등, 기본 모델 `깡통`)으로, 게스트가 들어오면 **사람 슬롯**(이름 텍스트 + AI 뱃지 소등)으로 자동 전환 — `participants` 배열에 존재 여부만으로 파생, 별도 상태 불필요

#### 멀티플레이 게임 시작 연동

- 호스트가 "게임 시작" 클릭 → `start-game`으로 플레이어 구성 + 게임 설정 전송 → 서버가 `room.started = true` 표시 후 `game-started`를 방 전체(호스트 포함)에 브로드캐스트
- 각 클라이언트는 `game-started` 수신 시 `las-vegas:playerConfig`/`las-vegas:gameSettings`를 **sessionStorage**(로컬스토리지 아님 — 같은 브라우저에서 호스트/게스트 탭을 나란히 테스트할 때 설정이 섞이지 않도록 탭별로 격리)에 저장 후 `/multi`로 이동

#### `/app/multi/page.tsx` 신규 생성

- 처음에는 `game/page.tsx`를 복사해 소켓 연결 스켈레톤만 갖춘 뼈대로 시작(서버 상태 수신 전엔 대기 화면만 렌더링)
- **버그 발견 및 수정**: 스켈레톤이 `io()`로 완전히 새 소켓을 만들어 로비가 이미 방에 join시켜 둔 소켓과 분리되는 바람에 `game-state` 브로드캐스트를 영영 받을 수 없었음 → 로비와 동일한 공유 소켓 싱글턴(`getSocket()`)을 재사용하도록 수정하고, 마운트 시 `request-game-state`로 현재 상태를 명시적으로 다시 요청하는 보정 경로를 추가(브로드캐스트-마운트 순서 경합 대비)

#### 서버 초기 게임 상태 생성 + 실시간 진행

- `buildInitialGameState()`: 싱글플레이와 동일한 `createInitialState()`/`distributeRound()`를 재사용해 1라운드 상태 생성, `game-state`로 브로드캐스트
- **주사위 굴리기**(`roll-dice`): `findColorOwnerSocketId()`(슬롯 색상 ↔ 참가자 소켓)로 현재 턴 소유자만 허용, `rollDice()`/`applyRoll()` 재사용
- **카지노 베팅**(`place-bet`): 굴린 눈에 해당하는 카지노인지 `getValidActions()`로 검증 후 `applyAction()` 재사용 — 턴 전환·주사위 소진 플레이어 자동 스킵은 엔진 함수 내부 로직 그대로 활용
- **LLM/깡통 자동 진행**: `scheduleLLMAutoRollIfNeeded()` ↔ `scheduleLLMAutoBetIfNeeded()`가 서로를 호출하며 굴림→베팅→다음 플레이어… 순으로 사람 개입 없이 연쇄 진행(현재 전부 무작위 선택 — 깡통과 실제 LLM 모델 구분 없음, 실제 LLM API 연동은 TODO)
- **라운드 정산**(`applyAction` 결과 phase가 `"scoring"`이 되는 순간): `applyScoring()`으로 소지금 갱신, `distributeRound()`로 다음 라운드 배치를 미리 계산해 `room.nextRoundPreview`에 캐싱(셔플이 매번 랜덤이라 정산 시점에 한 번만 계산 — 버튼 클릭 시 재계산하면 성공/실패가 달라질 수 있음). 배치 성공 시 `phase: "round-end"`, 실패(지폐 부족) 시 바로 `phase: "game-end"`
- **다음 라운드**(`next-round`, 호스트 전용): 캐싱해둔 `nextRoundPreview` 적용, 주사위 리셋, `humanFirst` 재셔플

#### 클라이언트 렌더링 (`/app/multi/page.tsx`)

- `phase`에 따라 완전히 분기: 대기 → 굴리기(내 턴만 버튼 활성) → 베팅(내 턴 + 굴린 눈에 해당하는 카지노만 클릭 가능) → 라운드 종료(플레이어 패널 + 다음 라운드[호스트 전용]/게임 종료 버튼) → 게임 종료(최종 소지금·우승자)
- 기존 싱글플레이 컴포넌트(`Casino`, `Die`, `PlayerPanel`)를 그대로 재사용, 정산/전환 애니메이션은 아직 없음(정적 표시만)

#### 기타

- `src/lib/socket-client.ts`: 앱 전역에서 공유하는 소켓 싱글턴 (`getSocket()`)
- `src/types/multiplayer.ts`: 방·게임 상태 관련 Socket.io 이벤트 페이로드 타입을 서버/클라이언트가 공유
- `src/app/game/page.tsx`(싱글플레이)는 이번 세션 동안 의도적으로 건드리지 않았다(사용자 명시 지시) — 단, 이전 세션에 이미 반영된 sessionStorage 우선 읽기 폴백(`readConfigItem`)은 그대로 유지됨
- 매 단계마다 `npx tsc --noEmit` + 실제 소켓(node 스크립트) 스모크 테스트로 검증 후 임시 스크립트 정리

### 현재 상태

방 생성부터 라운드 정산·다음 라운드 전환까지 멀티플레이 게임 한 사이클 전체가 서버 권위 방식으로 엔드투엔드 동작한다(전원 AI 자동 진행으로 실제 라운드 하나를 끝까지 돌려 정산·초기화·다음 라운드 전환까지 확인 완료). LLM 턴은 서버가 자동 처리하지만 현재는 깡통과 동일하게 무작위 선택이며, 실제 LLM API 판단은 아직 연동되지 않았다. 정산/라운드 전환 시각 연출은 없고 상태가 즉시 정적으로 반영된다. 게임 진행 중 플레이어 이탈·재접속, 턴 타임아웃, 게임 종료 후 정리(다른 플레이어 알림 등)는 아직 처리하지 않는다. `npx tsc --noEmit` 에러 없음 확인.

세션 종료 시점에는 미완이었으나, 아래 "다음 세션 예정 작업"은 모두 세션 11(`PROGRESS.md` 참고)에서 이어받아 완료했다.
...
- [ ] 에러 핸들링: LLM API 키 미설정, API 호출 실패 시 사용자 안내
- [ ] `npm audit` 경고 — Next.js 내부 postcss 취약점이나, `npm audit fix --force`하면 Next.js 9.x로 역행하므로 보류 중. Next.js 업스트림 패치 대기
- [ ] 멀티플레이 실제 LLM API 연동: 현재 서버의 LLM/깡통 자동 진행은 전부 무작위 선택 — `scheduleLLMAutoBetIfNeeded()`의 `TODO` 참고
- [ ] 멀티플레이 정산/라운드 전환 시각 연출 없음 — 싱글플레이의 정산 애니메이션(CLAUDE.md 항목 9~12) 미이식
- [ ] 멀티플레이 게임 진행 중 이탈/재접속 처리 없음 — `removeParticipant()`는 로비 단계 이탈만 다루고, 게임 시작 후(`room.started`) 이탈 시나리오는 별도 처리가 없음
- [ ] 턴 타임아웃 없음 — 사람 플레이어가 굴리기/베팅을 하지 않고 방치하면 게임이 무한정 멈춘다
- [ ] "게임 종료" 버튼이 임시로 `router.push("/")`만 수행 — 다른 플레이어에게 게임 종료를 알리는 서버 프로토콜 없음(이번 세션 범위 밖이라 개인 이탈로만 구현)
- [ ] NVIDIA NIM 모델 미지원 — LLM 프로바이더 목록에 없음

### 다음 세션 예정 작업 (사용자 지정 순서)

- **2~5단계 + 4단계 묶음**: 게임 종료 처리 + 이탈/종료 처리
- **5단계**: 턴 타임아웃 (15초)
- LLM 연동 서버 이전 (현재 서버의 무작위 자동 진행을 실제 LLM API 호출로 교체)
- 멀티 연출 추가 (싱글플레이 정산/전환 애니메이션 이식)
- NVIDIA NIM 모델 추가
